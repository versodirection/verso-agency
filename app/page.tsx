...
import { ArrowRight, Check, Mail, Phone, MapPin, ExternalLink, Menu, X, Loader2, Star, ChevronDown, Key } from "lucide-react";
...
const RealisationCard = ({ project }: { project: any }) => {
        ...
        {project.tech && (
          <div className="flex flex-wrap gap-2">
            {project.tech.map((t: string, i: number) => (
              <span key={i} className="px-3 py-1 text-xs font-semibold rounded-full border border-indigo-500/40 text-indigo-400 bg-indigo-500/10">{t}</span>
            ))}
          </div>
        )}
                ...
                >
                  <form onSubmit={handleDemoRequest} className="mt-4 flex flex-col sm:flex-row gap-3">
                    <input
                      ...
                      className="flex-1 px-4 py-2.5 rounded-xl bg-neutral-800 border border-white/10 text-white text-sm placeholder:text-neutral-500 focus:outline-none focus:border-indigo-500/50 transition-colors"
                    />
                    <button
                      type="submit"
                      disabled={demoStatus === 'loading' || demoStatus === 'success'}
                      className="px-5 py-2.5 rounded-xl bg-indigo-600 text-white text-sm font-semibold hover:bg-indigo-500 transition-colors disabled:opacity-50 flex items-center gap-2 justify-center cursor-pointer"
                    >
                  ...
                  {demoStatus === 'success' && (
                    <p className="text-green-400 text-xs mt-2">Nous reviendrons vers vous avec les accès.</p>
                  )}
                  {demoStatus === 'error' && (
